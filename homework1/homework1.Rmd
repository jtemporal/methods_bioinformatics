...
2) Download and load the 3 class annotation file (golubTrainClass2.txt) for this
data set into R.
```{r}
...
7) Now download and load the Spellman yeast data set.  Remember to set the row
names to the first column as you did before with the leukemia dataset.
```{r}

```

8) Cast the data to a data frame and subset to only work with the cdc28
experiment samples.
```{r}

```

9) Use both the function and call to the function below to fill all “NA” values
...
10) Calculate the kmeans clustering method on all 6,178 genes, using 10 cluster
centers and 100 iterations.
```{r}

```

11) Look for gene #2 (YAL002W) and find the cluster that it belongs to.  Using
...
12) Cast the distance object to a matrix and get the column that gene #2
...
13) Get the weights of each gene as a percentage of the sum of distance values.
